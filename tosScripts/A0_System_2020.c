...
AddLabel(
    1,
    if _rsiEmaDirection >= rsiBear                            # Direction is Trending Up
      ...
      then if directionZZSave >= 0                            # Direction ZZ is BUY (B1-4)
          then
              if zzSave >= GetValue(zzSave, 1)
                  then if zzSave >= 0 then Color.UPTICK else Color.Orange
              else
                  if zzSave >= 0 then CreateColor(3, 152, 252) else Color.Yellow        # Light Blue & Yellow
      else
          if zzSave >= GetValue(zzSave, 1)
                  then if zzSave >= 0 then CreateColor(140, 3, 252) else Color.Gray     # Purple & Gray
              else
                  if zzSave >= 0 then Color.Dark_Gray else Color.Gray
  else
      if directionZZSave < 0
              then
              if zzSave >= GetValue(zzSave, 1)
                  then if zzSave >= 0 then CreateColor(248, 3, 252) else Color.Yellow   # Hot Pink & Yellow
              else
                  if zzSave >= 0 then CreateColor(252, 136, 3) else Color.DOWNTICK      # Orange & Red
      else
          if zzSave >= GetValue(zzSave, 1)
                  then if zzSave >= 0 then Color.Gray else Color.Dark_Gray
              else
                  if zzSave >= 0 then Color.Gray else CreateColor(140, 3, 252)          # Gray & Purple
);
AddLabel(
    1,
    if GetValue(directionZZSave, 1) > directionZZSave
        then "D-Falling"
    else "D-Rising",
    if GetValue(directionZZSave, 1) > directionZZSave
        then Color.DOWNTICK
...
input mood = { Aggressive, default Conservative };

def _prey = if _rsiDirection >= rsiBear then prey.BUY
    else if _rsiDirection <= rsiBear then prey.SELL
    else prey;
...
def bearishConsolidation = if outlook == -1 then
    (_rsiDirection < _rsiEmaDirection) && (_rsiEmaDirection > Zero)
    else 0;
...
def buyTrig1 = Bullet crosses above triggerBuyLine;
...
def sellTrig1 = Bullet crosses below triggerSellLine;
...
plot StrongBUY_fire = Buy4;
plot StrongSELL_aim = Sell1;
plot StrongSELL_fire = Sell4;
...
AssignPriceColor(
    ...
    else if (StrongBUY_aim && (target == 1)) then Color.Green
    else if (StrongBUY_fire && (target == 1)) then Color.Cyan
    else if (StrongSELL_aim && (target == -1)) then Color.Red
