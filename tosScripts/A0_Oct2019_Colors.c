...
def RSI_BULL = ExpAverage(RSI(), TimeFrame * 8) - 50;
def RSI_BEAR = ExpAverage(RSI, TimeFrame * 5);
#===================================================================
...
def BuyZone = RSI_Bull > RSI_Bear && RSI > RSIBaseline;
def SellZone = RSI_Bull < RSI_Bear && RSI < RSIBaseline;
...
AssignPriceColor(
    ...
    else if StrongBUY_dn then Color.DARK_GREEN
    else if StrongBUY_up then Color.cyan
    else if StrongSELL_up then Color.Dark_Red
